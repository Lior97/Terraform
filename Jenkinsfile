pipeline {
  ...
  environment {
    GIT_CREDENTIALS = credentials('gitcreden')
    GIT_REPO = "github.com/Lior97/Terraform.git"
    APP_NAME = "java-mysql-app"
    IMAGE_REPO = "725666124077.dkr.ecr.eu-central-1.amazonaws.com"
    DOCKER_IMAGE_REPO = "liory97/jenkins"
      ...
      steps {
        script{
          dir("app") {
           ...
           //from some reason jenkins doesn't work well when multiple args are passed to docker build, this script bypasses that.
   ...
   //kubectl, aws-cli, eksctl and helm all need to be installed within the jenkins container prior to deploy stage .
    ...
    stage("push Image") {
      steps {
        script{
          dir("app"){
           withCredentials([usernamePassword(credentialsId: 'dockercreden', passwordVariable: 'PASS', usernameVariable: 'USER')])
           { 
            def version = readFile(file: 'version.txt')
            sh "echo $PASS | docker login -u $USER --password-stdin"
            def source_image = "${APP_IMAGE}:${version}"
            def target_image = "${DOCKER_IMAGE_REPO}:${version}"
